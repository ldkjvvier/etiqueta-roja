...
type StoreContextType = {
  ...
  generateWhatsAppMessage: (product?: Product, size?: string) => string
}
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const generateWhatsAppMessage = (product?: Product, size?: string) => {
    if (product && size) {
      ...
      // Cart order
      let message = "Hola Etiqueta Roja, me interesa hacer el siguiente pedido:\n\n"
      cartItems.forEach((item) => {
        message += `• ${item.name} - Talla: ${item.size} - Cantidad: ${item.quantity} - $${item.price * item.quantity}\n`
      })
      message += `\nTotal: $${cartTotal}`
      return `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(message)}`
    }
