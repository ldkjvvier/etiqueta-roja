...
export function CartSheet() {
  const { cartItems, isCartOpen, setIsCartOpen, removeFromCart, updateQuantity, cartTotal, generateWhatsAppMessage } =
    useStore()
    ...
    <Sheet open={isCartOpen} onOpenChange={setIsCartOpen}>
      <SheetContent className="w-full sm:max-w-md bg-background border-l border-border flex flex-col">
        <SheetHeader className="border-b border-border pb-4">
          <SheetTitle className="text-xl font-black tracking-tight">TU CARRITO ({cartItems.length})</SheetTitle>
        </SheetHeader>
        ...
        {cartItems.length === 0 ? (
          ...
          <>
            <div className="flex-1 overflow-y-auto py-4 space-y-4">
              {cartItems.map((item) => (
                <div key={`${item.id}-${item.size}`} className="flex gap-4 p-4 border border-border">
                  ...
                  <div className="flex-1 flex flex-col">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-muted-foreground text-xs mt-1">Talla: {item.size}</p>
                      </div>
                      <Button
                        variant="ghost"
                        size="icon"
